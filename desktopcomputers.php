                                    ...
                                    <div class="row">
                                        <div class="row1 col-lg-4">
                                            <form class="form-horizontal needs-validation" action="?do=Inserted" method="POST" enctype="multipart/form-data" novalidate>
                                                ...
                                                <a href="computers.php" class="linkfrom">Computers & Networking > Computers > Desktop Computers</a>
                                                ...
                                                <div class="row">
                                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-2">
                                                            <input id="model" type="hidden" value="Desktop Computers Comscll" name="typeclassifieds" class="forminput form-select-lg" autocomplete="off" required="required">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-group form-group-lg formmarginad"> 
                                                <div class="row">
                                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-2">
                                                            <input id="model" type="hidden" value="Computers & Networking > Computers > Desktop Computers" name="categoryy" class="forminput form-select-lg" autocomplete="off" required="required" placeholder="Model Of The cars">
                                                        </div>
                                                    </div>
                                                </div>
                                                <?php echo locate(); ?>
                                                ...
                                                <div class="row">
                                                        <div class="col-lg-6 col-md-8 col-sm-12 col-xs-2">
                                                        <textarea class=" form input_contact" name="description" rows="4" cols="38" required="required" placeholder="Describe your computer or device"></textarea>
                                                        <div class="invalid-feedback" style="text-align: initial;">
                                                            This field is required.
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="row ">
                                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-2">
                                                        <select class="form-select forminput" aria-label="Default select example" name="processorclassifieds" required >
                                                            <option value="">Processor Speed (Per Core)</option>
                                                            <?php
                                                                $allUsers = getAllFrom("*", "processorclassifieds", "", "", "Id");
                                                                foreach($allUsers as $user){
                                                                    echo "<option value='" . $user['Id'] . "'>" .$user['Name'] . "</option>";
                                                                }
                                                            ?>
                                                ...
                                                <div class="row ">
                                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-2">
                                                        <select class="form-select forminput" aria-label="Default select example" name="hardclassifieds" required >
                                                            <option value="">Hard Drive</option>
                                                            <?php
                                                                $allUsers = getAllFrom("*", "hardclassifieds", "", "", "Id");
                                                                foreach($allUsers as $user){
                                                                    echo "<option value='" . $user['Id'] . "'>" .$user['Name'] . "</option>";
                                                                }
                                                            ?>
        ...
        elseif($do == 'Inserted'){
            ...
            <?php echo Safetyfirst(); ?> 
                                            ...
                                            <?php
                                                                    ...
                                                                    <?php
                                                                            $hardclassifieds = $_POST['hardclassifieds'];
                                                                            $gethardclassifieds = $con->prepare("SELECT * FROM hardclassifieds WHERE Id = $hardclassifieds");
                                                                            $gethardclassifieds->execute();
                                                                            $info = $gethardclassifieds->fetch();
                                                                            $namehardclassifieds = $info['Name'];
                                                                            ?>    
                                                                    <input id="title" type="hidden" name="hardclassifieds" class="form-control form-select-lg" autocomplete="off" value="<?php echo $namehardclassifieds ?>" />
                                                                    <?php
                                                                            $processorclassifieds = $_POST['processorclassifieds'];
                                                                            $getprocessorclassifieds = $con->prepare("SELECT * FROM processorclassifieds WHERE Id = $processorclassifieds");
                                                                            $getprocessorclassifieds->execute();
                                                                            $info = $getprocessorclassifieds->fetch();
                                                                            $nameprocessorclassifieds = $info['Name'];
                                                                            ?>    
                                                                    <input id="title" type="hidden" name="processorclassifieds" class="form-control form-select-lg" autocomplete="off" value="<?php echo $nameprocessorclassifieds ?>" />
                                                                    <?php
                    ...
                    if($_SERVER['REQUEST_METHOD'] == 'POST')
                    {
                        ...
                        $categoryy     = $_POST['categoryy'];
                                    $stmtitemadd = $con->prepare("INSERT INTO
                                                            classifieds(Name, locationcars, categoryy, memoryclassifieds, processorclassifieds, hardclassifieds, categories, brandclassifieds, usageclassifieds, conditionclassifieds, warrantyclassifieds, ageclassifieds, date, Image, user_Id, phone, price, typeclassifieds, description, location, Approve)
                                                            VALUES(:zname, :zlocationcars, :zcategoryy, :zmemoryclassifieds, :zprocessorclassifieds, :zhardclassifieds, :zcategory, :zbrandclassifieds, :zusageclassifieds, :zconditionclassifieds, :zwarrantyclassifieds, :zageclassifieds, now(), :zimage, :zuser, :zphone, :zprice, :zclassifieds, :zdescription, :zlocation ,1)");
                                    $stmtitemadd->execute(array(
                                            'zlocationcars'     => $locationcars,
                                            'zcategoryy'     => $categoryy,
                                            'zmemoryclassifieds'     => $memoryclassifieds,
                                            'zprocessorclassifieds'     => $processorclassifieds,
                                            'zhardclassifieds'     => $hardclassifieds,
                                            'zname'     => $name,
                                            'zcategory' => $category,
