                                    ...
                                    <div class="row">
                                        <div class="row1 col-lg-4">
                                            <form class="form-horizontal needs-validation" action="?do=Inserted" method="POST" enctype="multipart/form-data" novalidate>
                                                ...
                                                <a href="motorcycleparts.php" class="linkfrom">Motorcycle Parts > Plumbing & Ventilation</a>
                                                ...
                                                <div class="row">
                                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-2">
                                                            <input id="model" type="hidden" value="Plumbing & Ventilation" name="typeparts" class="forminput form-select-lg" autocomplete="off" required="required">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-group form-group-lg formmarginad"> 
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST')
            {
                ...
                foreach($_FILES['image']['name'] as $key=>$val)
                {
                    ...
                    $imageExtension = strtolower(substr(strrchr($imageName, '.'), 1));
                        ...
                        // $image = rand(0, 100000000) . '_' . $_FILES['image']['name'][$key];
                        move_uploaded_file($imageTmp, "admin\Update_Imageparts\uploads\avatars\\" . $imageName);
                        $data   .=$imageName." ";
                }  
                $name       = $_POST['name'];
                $phone       = $_POST['phone'];
                    ...
                    if(empty($formErrors))
                    {
                        ...
                        if($check == 1)
                        {
                            ...
                            $stmtitemadd = $con->prepare("INSERT INTO
                                                    autoaccparts(Image, Name, phone, category, user_Id, sellertypeparts, Price, Add_date, Description, Location, usageparts, conditionparts, typeparts, extrasparts, Approve)
                                                    VALUES(:zimage, :zname, :zphone, :zcategory, :zuser, :zsellertypeparts, :zprice, now(), :zDescription, :zLocation, :zusageparts, :zconditionparts, :ztypeparts, :zextrascars, 1)");
